...
// @Description Get all distinct book categories
// @Tags        Categories
// @Accept      json
// @Produce     json
...
// @Description Search books by keyword in title, author, or description
// @Tags        Books
// @Accept      json
// @Produce     json
...
func searchBooks(c *gin.Context) {
	...
	rows, err := db.Query(`
		SELECT id, title, author, isbn, year, price, category, 
		       original_price, discount, cover_image, rating, reviews_count,
		       is_new, pages, language, publisher, description, created_at, updated_at
		FROM books 
...
// @Description Get featured books with high ratings
// @Tags        Books
// @Accept      json
// @Produce     json
...
// @Description Get books with active discounts
// @Tags        Books
// @Accept      json
// @Produce     json
// @Param       limit  query    int  false  "Number of books to return (default 10)"
...
func getDiscountedBooks(c *gin.Context) {
	rows, err := db.Query(`
		SELECT id, title, author, isbn, year, price, category, 
		       ...
		       is_new, pages, language, publisher, description, created_at, updated_at
		FROM books 
		WHERE discount > 0 AND original_price IS NOT NULL
...
func getAllBooks(c *gin.Context) {
	...
	if category == "" {
		...
		rows, err = db.Query("SELECT id, title, author, isbn, year, price, created_at, updated_at FROM books WHERE category = $1", category)
	}
